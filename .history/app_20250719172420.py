...
pt = power_transform(method='yeo-johnson')
import pickle
import pandas as pd
app = Flask(__name__)

# Sample car data
with open("xgboost2_model_pipeline.pkl", "rb") as f:
    model = pickle.load(f)
...
def predict():
        try:
            ...
            "make": request.form["company"],
            "model": request.form["modl"],
            ...
            dfm=df.select_dtypes(include='float')
            df[dfm.columns] = power_transform(dfm, method='yeo-johnson')
            pred=model.predict(df)
            print(pred)
            pred = pt.inverse_transform(pred)

            return render_template("index.html", prediction_text=f"Prediction: {pred}")
